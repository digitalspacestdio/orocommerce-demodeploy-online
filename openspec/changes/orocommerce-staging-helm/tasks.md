...
- [ ] 1.1 Create `deploy/charts/orocommerce/Chart.yaml`, `values.yaml`,
      `_helpers.tpl` with shared labels/env-building helpers.
- [ ] 1.2 Add `deploy/charts/orocommerce/templates/configmap.yaml` covering
      ...
      search DSNs `orm:...`).
- [ ] 1.3 Document (in `values.yaml` comments) the Secret keys the chart
      expects to be provided externally (`ORO_SECRET`, `ORO_DB_*`,
      `ORO_DUMP_URL`, `ORO_MAILER_DSN`) — no values committed.

## 2. Workloads

- [ ] 2.1 `templates/deployment-fpm.yaml` — php-fpm, image from
...
- [ ] 2.2 `templates/deployment-nginx.yaml` + `templates/configmap-nginx.yaml`
...
- [ ] 2.3 `templates/deployment-consumer.yaml` — same app image,
      `oro:message-queue:consume` command, single replica.
- [ ] 2.4 `templates/deployment-cron.yaml` — same app image, ofelia config
      (reuse `ofelia.conf`) driving `oro:cron` every minute.
- [ ] 2.5 `templates/deployment-websocket.yaml` — same app image,
      `gos:websocket:server`, DSNs per design.md decision 5.
- [ ] 2.6 `templates/deployment-mailpit.yaml` + Service (ClusterIP only by
      default; ingress only if a values flag requests it — design.md
      decision 6, open question 4).
- [ ] 2.7 `templates/deployment-gotenberg.yaml` + Service, gated by a
      `gotenberg.enabled` value (default per open question 3 — leave `true`
      with a comment that PO may set `false`).
- [ ] 2.8 `templates/statefulset-postgres.yaml` (or Deployment + PVC if a
...
- [ ] 2.9 `templates/services.yaml` — one ClusterIP Service per workload
      above, named so the nginx/app env DSNs in 2.2/2.5 resolve correctly.

## 3. Bootstrap Job

- [ ] 3.1 `templates/job-bootstrap.yaml` as a Helm
...
- [ ] 5.2 Add inline comments next to every value that depends on an open
...
- [ ] 6.1 `helm lint deploy/charts/orocommerce` passes.
- [ ] 6.2 `helm template deploy/charts/orocommerce -f
...
- [ ] 6.3 Confirm rendered output contains no secret values (only
      references to Secret/ConfigMap keys).
- [ ] 6.4 `docker build` for the existing `Dockerfile.prod`/`Dockerfile.nginx`
      still succeeds unmodified (chart does not require Dockerfile changes).
- [ ] 6.5 Open the PR against the repo default branch per DIG-3003's
